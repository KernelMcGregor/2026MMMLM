...
As with prior years, each game has a unique `ID` created by concatenating the season in which the game was played and the two team's respective `TeamId`s. For example, `2026_1101_1102` indicates a hypothetical matchup between team 1101 and 1102 in the year 2026. You must predict the probability that the team with the lower `TeamId` beats the team with the higher `TeamId`. Note that the men's teams and women's `TeamId`s do not overlap.
...
| March 19, 2026 4PM UTC | Final Submission Deadline |
| March 19 – April 6 | Watch your tournament results play out! |

> **Note:** Kaggle will release updated data at least once in advance of the deadline in order to include as much of the current season's data as possible. The organizers reserve the right to update the contest timeline if they deem it necessary.
